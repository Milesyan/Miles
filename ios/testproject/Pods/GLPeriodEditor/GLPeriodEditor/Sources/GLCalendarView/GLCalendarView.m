...
- (void)setup
{
    ...
    self.backToTodayButton.layer.cornerRadius = self.backToTodayButton.bounds.size.height / 2;
    self.backToTodayButton.layer.borderWidth = .5f;
//    self.backToTodayButton.layer.borderColor = [UIColor blueColor].CGColor;
    self.backToTodayButton.layer.shadowOffset = CGSizeMake(0, 0);
    self.backToTodayButton.layer.shadowRadius = 2;
    self.backToTodayButton.layer.shadowOpacity = 0.3;
    self.backToTodayButton.layer.masksToBounds = NO;
...
- (void)setupWeekDayTitle
{
    ...
    NSArray *titles = [GLDateUtils weekdayTitlesText];
...
- (void)reloadAppearance
{
    ...
    self.backToTodayButton.backgroundColor = appearance.backToTodayButtonColor ? appearance.backToTodayButtonColor : [UIColor darkGrayColor];
    self.backToTodayButton.layer.borderColor = appearance.backToTodayButtonBorderColor ? appearance.backToTodayButtonBorderColor.CGColor : [UIColor darkGrayColor].CGColor;
    
    if (appearance.backToTodayButtonImage) {
        [self.backToTodayButton setImage:appearance.backToTodayButtonImage forState:UIControlStateNormal];
    }
    if (self.backToTodayButtonDisplayOptions == HideBackToTodayButton) {
        self.backToTodayButton.hidden = YES;
    } else {
        self.backToTodayButton.hidden = NO;
    }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    // update backToTodayButton
    CGFloat delta = (self.bounds.size.height - self.backToTodayButton.center.y) / self.rowHeight;

    if (self.backToTodayButtonDisplayOptions != HideBackToTodayButton) {
        CGFloat offsetOverBottom = (self.collectionView.contentSize.height -
                                    self.collectionView.contentOffset.y -
                                    self.collectionView.bounds.size.height) / self.rowHeight;
        NSInteger rowsBetweenTodayAndLastDay = ceilf([GLDateUtils daysBetween:self.today and:self.lastDate] / 7);
    
        CGFloat rotation = atan2(offsetOverBottom - rowsBetweenTodayAndLastDay + delta, rowsBetweenTodayAndLastDay);
        self.backToTodayButton.transform = CGAffineTransformMakeRotation(-rotation);
        
        if (offsetOverBottom >= rowsBetweenTodayAndLastDay) {
            if (self.backToTodayButtonDisplayOptions & HideBackToTodayButtonBeforeToday) {
                self.backToTodayButton.alpha = 0;
            } else {
                self.backToTodayButton.alpha = 1;
            }
        } else {
            if (self.backToTodayButtonDisplayOptions & HideBackToTodayButtonAfterToday) {
                self.backToTodayButton.alpha = 0;
            } else {
                self.backToTodayButton.alpha = 1;
            }

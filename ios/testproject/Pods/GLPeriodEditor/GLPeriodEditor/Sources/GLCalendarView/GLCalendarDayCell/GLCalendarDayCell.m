...
//    NSLog(@"update ui: %@ %d", [GLDateUtils descriptionForDate:self.date], _enlargePoint);
    ...
    if ([self isToday]) {
        ...
        [self setMonthLabelText:NSLocalizedStringFromTable(@"Today", @"GLPeriodEditorLocalizedString", nil)];
        ...
        self.backgroundCover.backgroundImage = nil;
    } else if (day == 1) {
        self.monthLabel.textColor = [UIColor redColor];
        [self setMonthLabelText:[[GLDateUtils monthText:month] uppercaseStringWithLocale:[NSLocale currentLocale]]];
    ...
    if (self.range) {
        ...
        if (self.isToday && !self.range.backgroundImage) {
            self.dayLabel.textColor = [UIColor whiteColor];
            self.monthLabel.textColor = [UIColor whiteColor];
            self.backgroundCover.fillColor = self.range.textColor ?: self.range.backgroundColor;
            self.backgroundCover.backgroundImage = nil;
        } else {

...
- (void)updateWithFirstDate:(NSDate *)firstDate lastDate:(NSDate *)lastDate calendar:(NSCalendar *)calendar rowHeight:(CGFloat)rowHeight
{
    ...
    monthFormatter.dateFormat = [NSDateFormatter dateFormatFromTemplate:@"MMMM" options:0 locale:monthFormatter.locale];
    
    
    NSDateFormatter *monthYearFormatter = [[NSDateFormatter alloc] init];
    monthYearFormatter.locale = [NSLocale currentLocale];
    monthYearFormatter.dateFormat = [NSDateFormatter dateFormatFromTemplate:@"YYYYMMMM" options:0 locale:monthYearFormatter.locale];
    ...
    for (NSDate *date = [GLDateUtils monthFirstDate:firstDate]; [date compare:lastDate] < 0; date = [GLDateUtils dateByAddingMonths:1 toDate:date]) {
        ...
        {
            NSString *monthYearText = [monthYearFormatter stringFromDate:date];
            labelText = monthYearText;
        }

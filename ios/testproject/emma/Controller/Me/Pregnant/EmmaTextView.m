...
#import "UIWebView+Emma.h"

@implementation EmmaTextView
...
- (void)initSubviews
{
    ...
    NSURL *path = [[NSBundle mainBundle] URLForResource:@"editor" withExtension:@"html"];
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType
{
    ...
    if ([request.URL.scheme isEqualToString:@"keyboarddidshow"]) {
        if ([self.delegate respondsToSelector:@selector(emmaTextViewDidBeginEditing:)]) {
            [self.delegate emmaTextViewDidBeginEditing:self];
        }
        return NO;
    } else if ([request.URL.scheme isEqualToString:@"didblur"]) {
        if ([self.delegate respondsToSelector:@selector(emmaTextViewDidEndEditing:)]) {
            [self.delegate emmaTextViewDidEndEditing:self];
        }
        return NO;
        ...
        if ([self.delegate respondsToSelector:@selector(emmaTextViewDidChange:)]) {
            [self.delegate emmaTextViewDidChange:self];
        }
        return NO;
    }
    return YES;
...
{
    NSInteger i = self.insertedImages.count;
    NSString *imgId = [NSString stringWithFormat:@"img%ld", (long)i];

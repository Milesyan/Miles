...
- (void)loadView
{
    ...
    //migration from keychain to NSDefaults, which could be run repeated
    if ([[KKKeychain getStringForKey:@"passcode_on"] isEqualToString:@"YES"]) {
        [KKKeyDefaults setString:@"YES" forKey:@"passcode_on"];
        NSString* passcode = [KKKeychain getStringForKey:@"passcode"];
        [KKKeyDefaults setString:passcode forKey:@"passcode"];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    _passcodeLockOn = [[KKKeyDefaults getStringForKey:@"passcode_on"] isEqualToString:@"YES"];
	_eraseData = [[KKPasscodeLock sharedLock] eraseOption] && [[KKKeyDefaults getStringForKey:@"erase_data_on"] isEqualToString:@"YES"];
...
- (void)incrementFailedAttemptsLabel
{
    ...
    CGSize size = [_failedAttemptsLabel.text sizeWithAttributes:@{NSFontAttributeName: [Utils boldFont:self.isSmallLandscape ? 10.0f : 14.0f]}];
		...
		if (_mode == KKPasscodeModeSet) {
			if ([textField isEqual:_setPasscodeTextField]) {
				...
				if (![_confirmPasscodeTextField.text isEqualToString:_setPasscodeTextField.text]) {
					...
					if ([KKKeyDefaults setString:_setPasscodeTextField.text forKey:@"passcode"]) {
						[KKKeyDefaults setString:@"YES" forKey:@"passcode_on"];
					}
			...
			NSString* passcode = [KKKeyDefaults getStringForKey:@"passcode"];
...
- (void)vaildatePasscode:(UITextField*)textField
{
    if (_mode == KKPasscodeModeDisabled) {
        ...
        if ([_enterPasscodeTextField.text isEqualToString:passcode]) {
            if ([KKKeyDefaults setString:@"NO" forKey:@"passcode_on"]) {
                [KKKeyDefaults setString:@"" forKey:@"passcode"];
            }
...
- (UIView*)headerViewForTextField:(UITextField*)textField
{
	...
	headerLabel.font = [Utils boldFont:self.isSmallLandscape ? 12.0f : 17.0f];
	...
	if ([textField isEqual:_setPasscodeTextField]) {
		...
		_passcodeConfirmationWarningLabel.font = [Utils defaultFont:14.0];
	...
	if ([textField isEqual:_enterPasscodeTextField]) {
		...
		_failedAttemptsLabel.font = [Utils boldFont:self.isSmallLandscape ? 10.0f : 14.0f];
